...
def clean_schemas():
    schemas = []
    with open('/home/liujian/ccf/zy/pytorch/bin/bert-ie/datasets/all_50_schemas', encoding="utf8") as f:
        for l in tqdm(f):
            a = json.loads(l)
            if a['predicate'] != '丈夫':
                schemas.append(a)
    with open('/home/liujian/ccf/zy/pytorch/bin/bert-ie/datasets/all_50_schemas','w', encoding="utf8") as f:
        for sc in schemas:
...
def get_maybe_objs(train_objs, dev_objs, maybe_sub_dict_save_path=None,maybe_obj_dict_save_path=None):
    ...
    # with open( maybe_sub_dict_save_path , 'wb') as f:
    #     for i in maybe_sub_dict:
    #         maybe_sub_dict[i] = list(maybe_sub_dict[i])
    #     pk.dump( maybe_sub_dict , f )
    #
    # with open( maybe_obj_dict_save_path , 'wb') as f:
    #     pk.dump( maybe_obj_dict , f )
...
def add_no_sigs(objs,maybe_dict,maybe_type_dict,maybe_dict_without_tp,s_dic=None):
    ...
    # maybe_dict_without_tp['民族'] = set(filter( lambda x : '族' in x  and x!= '族',maybe_dict_without_tp['民族'])  )
    # maybe_dict['民族'] = set(filter(lambda x: '族' in x[0] and x[0]!= '族', maybe_dict['民族']))
    # clean_not_sig(objs, maybe_dict_without_tp['民族'], maybe_type_dict, '民族', maybe_dict, re_list,'民族',s_dic=s_dic )
    maybe_dict['祖籍'] = set()
    ...
    # maybe_dict['出品公司'] = set(filter(lambda x: '族' in x[0] and x[0] != '族', maybe_dict['民族']))
    # clean_not_sig(objs, maybe_dict_without_tp['出品公司'], maybe_type_dict, '民族', maybe_dict, re_list,'民族',s_dic=s_dic )
